...
export class SQLParser {
  ...
  private parseSelect(tokens: string[]): SelectQuery {
    ...
    if (
      ...
      tokens[currentIndex].toUpperCase() === "WHERE"
    ) {
      currentIndex++;
      const [cond, nextIdx] = this.parseConditionRecursive(
        tokens,
        currentIndex,
      );
      where = cond;
    ...
    if (
      currentIndex < tokens.length &&
      tokens[currentIndex].toUpperCase() === "LIMIT"
    ) {
      currentIndex++;
  ...
  private parseConditionRecursive(
    tokens: string[],
    idx: number,
  ): [Condition, number] {
  ...
  private parseAnd(tokens: string[], idx: number): [Condition, number] {
    ...
    while (
      ...
      tokens[nextIdx]?.toUpperCase() === "AND"
    ) {
      const operator = tokens[nextIdx].toUpperCase() as Operator;
  ...
  private parseOperand(tokens: string[], idx: number): [Condition, number] {
    ...
    if (!operator || !validOperators.includes(operator)) {
      throw new Error(
        `Missing or invalid operator in condition near '${left}'`,
      );
    }
    if (rightToken === undefined) {
      throw new Error(
        `Missing right-hand side in condition near '${left} ${operator}'`,
      );
    }
    if (validOperators.includes(rightToken)) {
      throw new Error(
        `Unexpected operator '${rightToken}' after operator '${operator}'`,
      );
    }
  ...
  execute(query: SelectQuery): Row[] {
    ...
    if (query.where) {
      results = results.filter((row) =>
        this.evaluateCondition(row, query.where!),
      );
    }
    ...
    if (
      ...
      const leftVal =
        typeof condition.left === "string"
          ? (row[condition.left] ?? condition.left)
          : this.evaluateCondition(row, condition.left);
      let rightVal: any;
      if (
